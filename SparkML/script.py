...
def main():
    ...
    pipeline_reg = Pipeline(stages=[imputer, assembler, trainer_reg])
    pipeline_bayes = Pipeline(stages=[imputer, assembler, trainer_bayes])
    pipeline_svm = Pipeline(stages=[imputer, assembler, trainer_svm])
    pipeline_forest = Pipeline(stages=[imputer, assembler, trainer_forest])
    ...
    print('\n', 'Best is Random Forrest!', '\n')

    paramGrid = ParamGridBuilder() \
        .addGrid(trainer_forest.numTrees, [10, 20, 30]) \
        .addGrid(trainer_forest.maxDepth, [3, 5, 7]) \
        .build()
    ...
    cv = CrossValidator(estimator=pipeline_forest,
    ...
    print()
    print(f"Best model: {cvModel.bestModel}")
    print(f"Average metrics: {', '.join([str(i) for i in cvModel.avgMetrics])}")

    rawPredictions = cvModel.transform(X_test)
    print("Best numTrees: " + str(cvModel.bestModel.stages[2].getNumTrees))
    print("Best maxDepth: " + str(cvModel.bestModel.stages[2].getMaxDepth()))
